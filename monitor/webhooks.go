...
func (t *Scraper) SendNewProdWebhook2(webhook string) {
	currentTime := time.Now()
	e := godiscord.NewEmbed(t.ProductTitle, "", t.BaseURL+"products/"+t.Handle)
	e.SetAuthor(t.BaseURL, "", "")
	e.SetColor("#257eb1")
	...
	e.AddField("Quicktasks", fmt.Sprintf("[Wrath](http://localhost:32441/qt?input=%vproducts/%v) | ", t.BaseURL, t.Handle) + fmt.Sprintf("[Balko](http://localhost:6776/?url=%vproducts/%v) | ", t.BaseURL, t.Handle) + fmt.Sprintf("[Nebula](http://localhost:7392/quicktask?url=%vproducts/%v) | ", t.BaseURL, t.Handle) + fmt.Sprintf("[Cyber](https://cybersole.io/dashboard/tasks?quicktask=%vproducts/%v) | ", t.BaseURL, t.Handle) + fmt.Sprintf("[Prism](https://dashboard.prismaio.com?url=%vproducts/%v) | ", t.BaseURL, t.Handle) + fmt.Sprintf("[Sole](https://www.soleaio.dev/quick_tasks/create?taskGroup=Shopify&product=%vproducts/%v)", t.BaseURL, t.Handle), false)
	e.SetFooter("Splash X • " + currentTime.Format("[01/02 15:04:05]")+ " • splash#0003", "https://i.ibb.co/PzyCsKx/splashxlogo.jpg")
	e.SendToWebhook(webhook)
	//fmt.Println(e.Content)
	//fmt.Println(e.Embeds)
}

func (t *Scraper) SendProductsRemovedWebhook2(webhook string) {
	currentTime := time.Now()
	e := godiscord.NewEmbed("", fmt.Sprintf("All Products pulled on %v", t.BaseURL), "https://i.ibb.co/PzyCsKx/splashxlogo.jpg")
	e.SetColor("#257eb1")
	e.SetAuthor(t.BaseURL, "", "")
	e.SetFooter("Splash X • " + currentTime.Format("[01/02 15:04:05]")+ " • splash#0003", "https://i.ibb.co/PzyCsKx/splashxlogo.jpg")
	e.SendToWebhook(webhook)
...
func (t *Scraper) SendPwPageUpWebhook2(webhook string) {
	currentTime := time.Now()
	e := godiscord.NewEmbed("", fmt.Sprintf("Password page up on %v", t.BaseURL), "https://i.ibb.co/PzyCsKx/splashxlogo.jpg")
	e.SetColor("#257eb1")
	e.SetAuthor(t.BaseURL, "", "")
	e.SetFooter("Splash X • " + currentTime.Format("[01/02 15:04:05]")+ " • splash#0003", "https://i.ibb.co/PzyCsKx/splashxlogo.jpg")
	e.SendToWebhook(webhook)
}
func (t *Scraper) SendPwPageDownWebhook2(webhook string) {
	currentTime := time.Now()
	e := godiscord.NewEmbed("", fmt.Sprintf("Password page down on %v", t.BaseURL), "https://i.ibb.co/PzyCsKx/splashxlogo.jpg")
	e.SetColor("#257eb1")
	e.SetAuthor(t.BaseURL, "", "")
	e.SetFooter("Splash X • " + currentTime.Format("[01/02 15:04:05]")+ " • splash#0003", "https://i.ibb.co/PzyCsKx/splashxlogo.jpg")
	e.SendToWebhook(webhook)
